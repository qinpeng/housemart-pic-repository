...
import org.housemart.pic.service.crawl.anjuke.search.CommunitySearcher;
import org.housemart.pic.service.crawl.anjuke.search.HouseListSearcher;
import org.housemart.pic.service.crawl.anjuke.store.data.DataConverter;
import org.housemart.pic.service.crawl.anjuke.store.file.PicCrawler;
  ...
  private void run(List<ResidenceEntity> residencesToCrawl) {
    ...
    for (ResidenceEntity residence : residencesToCrawl) {
      ...
      if (houseService.isResidenceCrawled(residence.getResidenceId())) {
        log.info("AnJuKeResidence::" + residence.getPlateName() + "::" + residence.getResidenceName() + "-已抓取，忽略");
        continue;
      }
      
      log.info("AnJuKeResidence::" + residence.getPlateName() + "::" + residence.getResidenceName() + "-即将抓取");
      
      String pathPrefix = residence.getRegionName() + "/" + residence.getPlateName() + "/" + residence.getResidenceName();
      
      try {
        
        List<Property> properties = houseListSearcher.search(AnJuKeCrawlConstants.CITY_SHANGHAI, residence.getResidenceName()
            .trim());
        if (CollectionUtils.isNotEmpty(properties)) {
          ...
          if (CollectionUtils.isNotEmpty(photosOfResidence)) {
            for (String photo : photosOfResidence) {
              ...
              String storePath = subPath + "/" + StringUtils.substringAfterLast(url, "com/").replaceAll("/", "-");
              // file
              picCrawler.crawl(url, storePath);
              
              // db
              HousePicEntity.CrawlStatus crawlStatus = CrawlStatus.Error;
              if (picCrawler.getScraper().getStatus() == Scraper.STATUS_FINISHED) crawlStatus = CrawlStatus.Success;
              addHousePic(null, residence.getResidenceId(), storePath, HousePicEntity.Type.Residence, crawlStatus);
            }
          }
          ...
          for (Property property : properties) {
            ...
            if (CollectionUtils.isNotEmpty(photosOfHouse)) {
              for (ModelPhotos photo : photosOfHouse) {
                ...
                HousePicEntity.CrawlStatus crawlStatus = CrawlStatus.Error;
                if (picCrawler.getScraper().getStatus() == Scraper.STATUS_FINISHED) crawlStatus = CrawlStatus.Success;
                addHousePic(houseId, residence.getResidenceId(), storePath, HousePicEntity.Type.RoomType, crawlStatus);
              }
            }
            ...
            if (CollectionUtils.isNotEmpty(photosOfRoom)) {
              for (RoomPhotos photo : photosOfRoom) {
                ...
                HousePicEntity.CrawlStatus crawlStatus = CrawlStatus.Error;
                if (picCrawler.getScraper().getStatus() == Scraper.STATUS_FINISHED) crawlStatus = CrawlStatus.Success;
                addHousePic(houseId, residence.getResidenceId(), storePath, HousePicEntity.Type.HousePic, crawlStatus);
              }
            }
