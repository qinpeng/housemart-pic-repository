...
public class WorkBench {
  ...
  private void run(List<ResidenceEntity> residencesToCrawl) {
    ...
    for (ResidenceEntity residence : residencesToCrawl) {
      ...
      if (houseService.isResidenceCrawled(residence.getResidenceId())) {
        log.info("AnJuKeResidence::" + residence.getPlateName() + "::"
            + residence.getResidenceName() + "-已抓取，忽略");
        continue;
      }
      
      log.info("AnJuKeResidence::" + residence.getPlateName() + "::"
          + residence.getResidenceName() + "-即将抓取");
      
      String pathPrefix = residence.getRegionName() + "/"
          + residence.getPlateName() + "/" + residence.getResidenceName();
      
      try {
        
        List<Property> properties = houseListSearcher.search(
            AnJuKeCrawlConstants.CITY_SHANGHAI, residence.getResidenceName()
                .trim());
        if (CollectionUtils.isNotEmpty(properties)) {
          ...
          CommunityGet communityGet = communitySearcher
              .search(anjukeCommunityId);
          ...
          if (CollectionUtils.isNotEmpty(photosOfResidence)) {
            for (String photo : photosOfResidence) {
              String url = photo;
              String subPath = pathPrefix
                  + WorkBenchConstants.FILE_SUBPATH_RESIDENCE;
              ...
              addHousePic(null, residence.getResidenceId(), storePath,
                  HousePicEntity.Type.Residence, crawlStatus);
            }
          }
          ...
          for (Property property : properties) {
            ...
            HouseEntity house = DataConverter
                .convertPropertyToHouseEntity(property);
            ...
            if (CollectionUtils.isNotEmpty(photosOfHouse)) {
              for (ModelPhotos photo : photosOfHouse) {
                ...
                String subPath = pathPrefix
                    + WorkBenchConstants.FILE_SUBPATH_ROOM;
                String storePath = subPath
                    + "/"
                    + StringUtils.substringAfterLast(url, "com/").replaceAll(
                        "/", "-");
            ...
            if (CollectionUtils.isNotEmpty(photosOfRoom)) {
              for (RoomPhotos photo : photosOfRoom) {
                String url = photo.getUrl();
                String subPath = pathPrefix
                    + WorkBenchConstants.FILE_SUBPATH_HOUSE;
                String storePath = subPath
                    + "/"
                    + StringUtils.substringAfterLast(url, "com/").replaceAll(
                        "/", "-");
  ...
  private void addHousePic(Integer houseId, int residenceId, String path,
      HousePicEntity.Type type, HousePicEntity.CrawlStatus crawlStatus) {
